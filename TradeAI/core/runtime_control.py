    ...
    try:
        ...
        for attempt in range(12):
            try:
                ...
                # Windows can surface sharing violations as PermissionError or
                # generic OSError with winerror 5/32 while the UI is reading.
...
class EngineInstanceLock:
    """Cross-process single-engine guard.

    TradeAI uses shared logs, reports, runtime status and command files. Running a
    backtest and DEMO_FORWARD engine at the same time corrupts that shared control
    plane. This lock deliberately permits only one demo_bot.py process at a time.
    """
    ...
    def acquire(self) -> None:
        ...
        active = RuntimeControlPlane.active_instance(clean_stale=True)
        raise RuntimeError(
            f"TradeAI engine instance lock is busy"
...
class RuntimeControlPlane:
    ...
    @staticmethod
    def issue_command(command: str, source: str = "desktop-ui", target_session_id: str = "") -> int:
    ...
    def active_instance(*, clean_stale: bool = True) -> dict[str, Any]:
        ...
        # A just-created exclusive lock can briefly exist before its JSON body is
        # flushed. Treat that short window as active instead of deleting it.
        try:
